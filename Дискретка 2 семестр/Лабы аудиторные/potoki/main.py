...
def updateV(V, T, f):
    for t in T:
        ...
        direct = V[t[2]][t[1]][2]  # направление движения

        # меняем веса в таблице для (i,j) и (j,i)
        V[t[1]][t[2]][0] -= f * direct
        V[t[1]][t[2]][1] += f * direct

        V[t[2]][t[1]][0] -= f * direct
        V[t[2]][t[1]][1] += f * direct
...
#матрица потоков ребра [использованное проходимость,остаток проходимости,направление по стрелке]


N = len(V)
init = 0    # вершина истока 
end = 4     # вершина стока
Tinit = (math.inf, -1, init)   #певая вершина
...
while j != -1:
    ...
    while k != end:     # пока не дошли до стока
        j = get_max_val(k, V, S)  # выбираем вершину с наибольшей пропускной способностью
        ...
        if j == end:    # если дошли до стока
            f.append(get_max_potok(T))     # находим максимальную пропускную способность маршрута
